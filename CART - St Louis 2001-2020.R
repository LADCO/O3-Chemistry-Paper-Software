############################################################################################
#                                                                                          #
#               RUNNING CART FOR ST. LOUIS  - FOR DATA 2001-2020 (May-Sept)             #
#                          USING MET DATA FROM ST. LOUIS AIRPORT                           #
...
## IMPORT DATA - ST. LOUIS ANALYSIS (WITH ST. LOUIS AIRPORT MET) ##############################

O3.met <- read.csv("./MetDat/Ozone-met files 2001-20/Ozone-met data for CART 2001-20 -St Louis.csv", header=TRUE)
...
## DO A FEW EXPLORATORY PLOTS 
    ...
    O3.Tmax.plot <- ggplot(O3.met, aes(x=tmax, y=MDA8)) + geom_point() + ggtitle("Ozone-Tmax plot - St Louis, 2001-2020") +
      xlab("Maximum daily Temperature (F)") + ylab("MDA8 ozone (ppb)")
    ggsave(filename = "./2001-2020 CART - for W-W/Ozone-Tmax plot - St Louis 2001-2020.png", plot = O3.Tmax.plot, width = 6, height =4)

    O3.hist <- ggplot(O3.met, aes(x=MDA8)) + geom_histogram(binwidth=2, color="black", fill="white") +
      xlab("MDA8 ozone (ppb)") + ylab("Count") + ggtitle("MDA8 Ozone - St Louis, 2001-2020")
    ggsave(filename = "./2001-2020 CART - for W-W/Ozone histogram - St Louis 2001-2020.png", plot = O3.hist, width = 6, height =4)
...
#Check temperature distribution in different years. (Had problems with this for Milwaukee and Chicago O'Hare in 2015)
    T.hist.by.yr <- ggplot(O3.met, aes(x=tavgpm)) + geom_histogram(binwidth=2, color="black", fill="white") + facet_wrap(. ~ year) +
      xlab("pm T") + ylab("Count") + ggtitle("Avg pm temp - St Louis, 2001-2020")
    ggsave(filename = "./2001-2020 CART - for W-W/pm temp histogram by yr - St Louis 2001-2020.png", plot = T.hist.by.yr, width = 6, height =4)
    
# #Check the distribution of nodes in different years. (as a way of checking temperature) - CAN ONLY RUN THIS AFTER RUNNING THE SCRIPTS BELOW
#     node.hist.by.yr <- ggplot(O3.met, aes(x=node)) + geom_histogram(binwidth=1, color="black", fill="white") + facet_wrap(. ~ year) +
#       xlab("node") + ylab("Count") + ggtitle("node distribution - St Louis, 2001-2020")
#     ggsave(filename = "./2001-2020 CART - for W-W/node histogram by yr - St Louis 2001-2020.png", plot = node.hist.by.yr, width = 6, height =4)
...
tree2 <- ctree(MDA8 ~ dpavg + dpmax + foghrs + hazehrs + mrmax + precip + rainhrs + rhavg + rhavgmid + rhavgnight + slpavg + stpavg + 
               ...
               data = O3.met.2001.10,
               # control = ctree_control(maxdepth=5, minsplit=600, minbucket=300, maxsurrogate=3))
               control = ctree_control(maxdepth=5, minsplit=800, minbucket=400, maxsurrogate=3))
...
print(tree2)
    ...
    # plot.new()
    # png(file="./2001-2020 CART - for W-W/Cond-Inf CART ctree - St Louis 01-20.png", width=3200, height=1800, res=200)
    # plot(tree2)
    # dev.off()
...
# write.csv(node.day, "St Louis CART data - 2001-2020.csv", row.names = FALSE)
...
# write.csv(select.met.data, "St Louis CART data - nodes-select met - 2001-2020.csv", row.names = FALSE)
...
#Plot boxplots of ozone in each node for each set of years
    ...
    #   ggtitle("St. Louis - MDA8 Ozone in CART Nodes \nCART run using 2001-2010 data") +
    ...
    # ggsave(filename = "CART boxplots by nodes and year set - 2001-20 - St Louis.png", plot = a, width = 8, height =5)
...
nodes.gt70 <- O3.met.all %>%
  ...
  ungroup()
write.csv(nodes.gt70, "CART 2001-20 - days gt70 in each node - St. Louis.csv", row.names=FALSE)

#Plot histogram
    # a <- ggplot(data = O3.met.all) + facet_wrap(~ node) +
    #   geom_histogram(aes(MDA8), binwidth = 2) + geom_vline(aes(xintercept = 70), color="darkgray") +
    #   xlab("MDA8 ozone (ppb)") + ylab("Frequency") + ggtitle(paste0("Histogram of MDA8 ozone by CART node-St. Louis 2001-20")) +
    ...
    # ggsave(filename = paste0("Histogram of MDA8 ozone by node - St Louis 2001-20.png"), plot=a, width = 9, height = 6.5)
...
high.O3.avg.node <- filter(O3.avg.node, node %in% c(14,20,21,27)) #selects just nodes with average O3>50 ppb 
# high.O3.avg.node <- filter(O3.avg.node, node %in% c(8,9,11)) #selects just nodes with average O3>50 ppb - without 2015
...
#Mean ozone plot
    O3.node.trend.plot <- ggplot(high.O3.avg.node, aes(x=year,y=mean.O3, color=node)) + geom_point() + geom_smooth(method=lm, se = FALSE , size=0.75) +
      ...
      scale_color_brewer(palette="Set1") + ggtitle("2001-2020 Trends by CART Node: St. Louis") +
      annotate("text", x= 2008, y=80, label="Only nodes with ozone >60 ppb in 2001-10", size = 4, hjust=0, vjust=1) +
      theme(axis.text = element_text(size = 14, color = "black"),
                    axis.title = element_text(size = 16),
                    legend.text = element_text(size = 12),
                    legend.title = element_text(size = 12),
                    plot.title = element_text(size = 15, hjust = 0))
    # ggsave(filename="./2001-2020 CART - for W-W/O3 trends 2001-20 St Louis means-minbuck 700.png", plot=O3.node.trend.plot, width=7, height=4)
    ggsave(filename="./2001-2020 CART - for W-W/O3 trends 2001-20 St Louis means-minbuck 400 minsplit 800.png", plot=O3.node.trend.plot, width=7, height=4)
...
#Plot results
    png("./2001-2020 CART - for W-W/variable importance 2001-10 St Louis.png", height=500, width=300)
...
tree.caret.var.imp <- varImp(tree.caret)
    ...
    # Plot results
    # png("./2001-2020 CART - for W-W/variable importance 2001-20 St Louis.png", height=500, width=300)
    # plot(tree.caret.var.imp, top=20)
    # dev.off()
...
tree.caret.var.imp <- varImp(tree.caret)
    ...
    #Plot results
    # png("./2001-2020 CART - for W-W/variable importance 2011-20 St Louis.png", height=500, width=300)
    # plot(tree.caret.var.imp, top=20)
    # dev.off()
