############################################################################################
#                                                                                          #
#           RUNNING CART FOR PORTER & Lake COs., IN  - FOR 2001-2020 (May-Sept)            #
#                            USING MET DATA FROM CHICAGO O'HARE                            #
...
O3.met <- read.csv("Ozone-met data for CART 2001-20 -Chicago- Lake-Porter IN.csv", header=TRUE)
...
## DO A FEW EXPLORATORY PLOTS
    ...
    O3.Tmax.plot <- ggplot(O3.met, aes(x=tmax, y=MDA8)) + geom_point() + ggtitle("Ozone-Tmax plot - Lake-Porter, 2001-2020") +
      xlab("Maximum daily Temperature (F)") + ylab("MDA8 ozone (ppb)")
    ggsave(filename = "./2001-2020 CART - for W-W/Ozone-Tmax plot - Lake-Porter v OHare 2001-2020.png", plot = O3.Tmax.plot, width = 6, height =4)

    O3.hist <- ggplot(O3.met, aes(x=MDA8)) + geom_histogram(binwidth=2, color="black", fill="white") +
      xlab("MDA8 ozone (ppb)") + ylab("Count") + ggtitle("MDA8 Ozone - Lake-Porter, 2001-2020")
    ggsave(filename = "./2001-2020 CART - for W-W/Ozone histogram - Lake-Porter v OHare 2001-2020.png", plot = O3.hist, width = 6, height =4)

##Discovered that 2015 ORD met has temperatures that are REALLY off - way too high. Already eliminated data 
...
print(tree2)
    ...
    plot.new()
    png(file="./2001-2020 CART - for W-W/Cond-Inf CART ctree - Lake-Porter 01-20 OHare met.png", width=2800, height=1500, res=200)
    plot(tree2)
    dev.off()
...
# write.csv(node.day, "Chicago Lake-Porter CART data - 2001-2020.csv", row.names = FALSE)
...
# write.csv(select.met.data, "Chicago Lake-Porter CART data - nodes-select met - 2001-2020.csv", row.names = FALSE)
...
#Plot boxplots of ozone in each node for each set of years
    ...
    #   ggtitle("Chicago: Lake-Porter - MDA8 Ozone in CART Nodes \nCART run using 2001-2010 data") +
    ...
    # ggsave(filename = "CART boxplots by nodes and year set - 2001-20 - Chicago Lake-Porter.png", plot = a, width = 8, height =5)
...
nodes.gt70 <- O3.met.all %>%
  ...
  ungroup()
write.csv(nodes.gt70, "CART 2001-20 - days gt70 in each node - Chicago Lake-Porter.csv", row.names=FALSE)

#Plot histogram
    # a <- ggplot(data = O3.met.all) + facet_wrap(~ node) +
    #   geom_histogram(aes(MDA8), binwidth = 2) + geom_vline(aes(xintercept = 70), color="darkgray") +
    #   xlab("MDA8 ozone (ppb)") + ylab("Frequency") + ggtitle(paste0("Histogram of MDA8 ozone by CART node-Chicago: Lake-Porter 2001-20")) +
    ...
    # ggsave(filename = paste0("Histogram of MDA8 ozone by node - Chicago Lake-Porter 2001-20.png"), plot=a, width = 9, height = 6.5)
...
#Didn't need to eliminate any years with just two days 
...
high.O3.avg.node <- filter(O3.avg.node, node %in% c(24,27,28)) #selects just nodes with average O3>=58 ppb 
...
#Mean ozone plot
    ...
    #   scale_color_brewer(palette="Set1") + ggtitle("2001-2020 Trends by CART Node: Lake-Porter Counties") +
    #   annotate("text", x= 2008, y=77, label="Only nodes with ozone >58 ppb in 2001-2010", size = 4, hjust=0, vjust=1) +
    #   theme(axis.text = element_text(size = 14, color = "black"),
    #                 axis.title = element_text(size = 16),
    #                 legend.text = element_text(size = 12),
    #                 legend.title = element_text(size = 12),
    #                 plot.title = element_text(size = 15, hjust = 0))
    # ggsave(filename="./2001-2020 CART - for W-W/O3 trends 2001-20 Lake-Porter means - OHare met.png", plot=O3.node.trend.plot, width=7, height=4)
    # # ggsave(filename="./2001-2020 CART - for W-W/O3 trends 2001-20 Lake-Porter means - OHare met-minbuck 150.png", plot=O3.node.trend.plot, width=7, height=4)
...
tree.caret.var.imp <- varImp(tree.caret)
    ...
    # #Plot results
    # png("./2001-2020 CART - for W-W/variable importance 2001-10 Chicago Lake-Porter.png", height=500, width=300)
...
tree.caret.var.imp <- varImp(tree.caret)
    ...
    # # Plot results
    #   png("./2001-2020 CART - for W-W/variable importance 2001-20 Chicago Lake-Porter.png", height=500, width=300)
    #   plot(tree.caret.var.imp, top=20)
    #   dev.off()
...
tree.caret.var.imp <- varImp(tree.caret)
    ...
    # Plot results
    png("./2001-2020 CART - for W-W/variable importance 2011-20 Chicago Lake-Porter.png", height=500, width=300)
    plot(tree.caret.var.imp, top=20)
    dev.off()
